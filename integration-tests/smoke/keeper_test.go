...
var _ = Describe("Keeper v1.2 Migrate upkeep from a registry to another @keeper", getKeeperSuite(ethereum.RegistryVersion_1_2, defaultRegistryConfig, BasicCounter, MigrateUpkeepTest))
		...
		Describe("with Keeper job", func() {
				...
				It("removes one keeper and makes sure the upkeeps are not affected by this and still perform", func() {
					...
					Eventually(func(g Gomega) {
						for upkeepID := 0; upkeepID < len(upkeepIDs); upkeepID++ {
							...
							g.Expect(counter.Int64()).Should(BeNumerically(">", int64(0)),
								"Expected consumer counter to be greater than 0, but got %s", counter)
						}
					}, "1m", "1s").Should(Succeed())
					...
					Eventually(func(g Gomega) {
						for i := 0; i < len(upkeepIDs); i++ {
							...
							g.Expect(counter.Int64()).Should(BeNumerically(">", initialCounters[i]),
								"Expected consumer counter to be greater than initial counter which was %s, but got %s", initialCounters[i], counter)
						}
					}, "1m", "1s").Should(Succeed())
				})
			}
			...
			if testToRun == MigrateUpkeepTest {
				It("creates another registry and migrates one upkeep to the new registry", func() {
					...
					Expect(err).ShouldNot(HaveOccurred(), "Couldn't cancel the upkeep on the first registry")
					err = networks.Default.WaitForEvents()
					Expect(err).ShouldNot(HaveOccurred(), "Failed to wait for events before the migration")

					// Get the balance of the upkeep after it was cancelled
					upkeepInfo, err := registry.GetUpkeepInfo(context.Background(), big.NewInt(0))
					Expect(err).ShouldNot(HaveOccurred(), "Failed to retrieve upkeep info before migration")

					// Get the counter after upkeep was cancelled
					//counterBeforeTheMigration, err := consumers[0].Counter(context.Background())
					//Expect(err).ShouldNot(HaveOccurred(), "Couldn't retrieve the counter before the migration")
					...
					actions.RegisterUpkeepContracts(linkToken, upkeepInfo.Balance, networks, upkeepGasLimit,
					...
					Eventually(func(g Gomega) {
						upkeepInfoAfterMigration, err := secondRegistry.GetUpkeepInfo(context.Background(), big.NewInt(5))
						g.Expect(err).ShouldNot(HaveOccurred(), "Failed to get upkeep info after migration")
						// We want the balance to be lower than before, meaning the upkeep is getting executed
						g.Expect(upkeepInfoAfterMigration.Balance).Should(BeNumerically("<", upkeepInfo.Balance),
							"Expected balance to be less than %s, but got %s", upkeepInfo.Balance, upkeepInfoAfterMigration.Balance)
					}, "1m", "1s").Should(Succeed())
				})
			}
		})
